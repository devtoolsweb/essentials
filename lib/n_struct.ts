...
import { IBaseClass, IBaseClassOpts } from './base_class'
import { IDisposable } from '@devtoolsweb/ts-goodies'
...
export function NStructChildMixin<TBase extends Constructor<IDisposable>> (Base: TBase): TBase & Constructor<INStructChild> {
    return class MixedNStructChild extends Base implements INStructChild, IChild {

        get chain (): INStructChild[] {
            const a = []
            let p: INStructChild | null = this as INStructChild
            while (p) {
                a.unshift(p)
                p = p.parent
            }
...
> (Base: TBase): TBase & INStructContainerConstructor<INStructContainer<T>> & INStructChildConstructor {
     ...
     */
        ...
        findChild (predicate: (c: T) => boolean) {
            let child = null
            this.enumChildren((c: T): NStructVisitorResult => {
                if (predicate(c)) {
                    child = c
                    return 'Leave'
                }
        ...
        *iterator () {
        ...
        truncate (last = 0): this {
            ...
            while (this.childCount > index) {
                this.removeChild(this.lastChild as T)
            }
            return this
        }
...
export const StandardNStructChild = (base: Constructor<IBaseClass>): Constructor<IDisposable> & Constructor<INStructChild> & Constructor<IBaseClass> =>
